...
\begin{document}
	...
	\section{Anwendungsarchitekturen}
		...
		\subsection{Informationssysteme}
			...
			\end{itemize}
		\begin{figure}
			\begin{tikzpicture}[centering text, text width=\textwidth]
			\matrix {
				\node (p1) [block] {\tiny Benutzerschnittstelle}; \\
				\node (p2) [block] {\tiny Benutzerkommunikation \& Authentifizierung und Autorisation}; \\
				\node (p3)  [block]  {\tiny Abrufen und Ändern von Informationen}; \\
				\node (p4) [block] {\tiny Transaktionsverwaltung \\ Datenbank}; \\
			};
			\end{tikzpicture}
		...
		\subsection{Sprachverarbeitende Systeme}
			...
			\begin{figure}
				...
				\matrix[ampersand replacement=\&, column sep = 5mm, row sep = 5mm] {
					\node (p1) [block] {\tiny Anweisungen der Quellsprache}; \& \node (p2) [block2] {\tiny Übersetzer: \\ Syntax prüfen; \\ Semantik prüfen; \\ Erstellen}; \& \\
					\& \node (p3)  [block2]  {\tiny abstrakte Maschinenanweisungen}; \& \\
					\node (p4) [block] {\tiny Daten}; \& \node (p5) [block2] {\tiny \textbf{Interpreter:} \\ abrufen; \\ ausführen}; \&
